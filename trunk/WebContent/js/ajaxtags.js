...
AjaxJspTag.Base = Class.create({
    ...
    buildParameterString: function (ajaxParam) {
        var result = [], field,formvalue,key, v;
        ...
        params.split(',').each(function (pair) {
            ...
            pair = pair.split('=');
            if(pair[0]=='formid'){   	
            	formvalue=$(pair[1].substring(1, pair[1].length - 1)).serialize(true);
            	for(var prefix in formvalue){
            		result.push(prefix+"="+encodeURIComponent(formvalue[prefix]));
            	}
            	return;
            }
            key = pair[0].strip() + '=';
